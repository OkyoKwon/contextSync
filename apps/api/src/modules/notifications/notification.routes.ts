...
export const notificationRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/users/me/notification-settings', async (request, reply) => {
    const settings = await getUserNotificationSettings(app.localDb, request.user.userId);
  ...
  app.put<{ Body: unknown }>('/users/me/notification-settings', async (request, reply) => {
    ...
    const settings = await updateUserNotificationSettings(app.localDb, request.user.userId, input);
    return reply.send(ok(settings));
  });
};
