...
describe('detectFileConflicts', () => {
  ...
  it('should assign warning severity for 3-5 overlapping files', () => {
    const newSession = {
      id: 'session-new',
      userId: 'user-a',
  ...
  it('should assign critical severity for 6+ overlapping files', () => {
    const paths = Array.from({ length: 7 }, (_, i) => `src/file${i}.ts`);
...
describe('determineSeverity', () => {
  it('should return info for 1-2 overlaps', () => {
    expect(determineSeverity(1)).toBe('info');
    expect(determineSeverity(2)).toBe('info');
  });

  it('should return warning for 3-5 overlaps', () => {
    expect(determineSeverity(3)).toBe('warning');
    expect(determineSeverity(5)).toBe('warning');
  });

  it('should return critical for 6+ overlaps', () => {
    expect(determineSeverity(6)).toBe('critical');
