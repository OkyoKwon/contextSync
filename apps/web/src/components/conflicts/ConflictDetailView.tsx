  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-6">
      <div className="flex items-center gap-3">
        ...
        <span className="ml-auto text-sm text-gray-500">{formatDate(conflict.createdAt)}</span>
      </div>

      <p className="mt-4 text-sm text-gray-700">{conflict.description}</p>

      <div className="mt-4">
        <h4 className="text-sm font-medium text-gray-900">Overlapping Files</h4>
        <div className="mt-2 space-y-1">
          {conflict.overlappingPaths.map((path) => (
            <div key={path} className="rounded bg-red-50 px-3 py-1.5 text-sm font-mono text-red-800">
              {path}
            </div>
          ))}
        </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-4">
        <div className="rounded-lg border border-gray-200 p-4">
          <h4 className="text-sm font-medium text-gray-900">Session A</h4>
          <p className="mt-1 text-xs text-gray-500">ID: {conflict.sessionAId}</p>
          {conflict.sessionATitle && <p className="text-sm">{conflict.sessionATitle}</p>}
        </div>
        <div className="rounded-lg border border-gray-200 p-4">
          <h4 className="text-sm font-medium text-gray-900">Session B</h4>
          <p className="mt-1 text-xs text-gray-500">ID: {conflict.sessionBId}</p>
