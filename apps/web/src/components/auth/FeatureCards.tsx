const FEATURES = [
  {
    ...
    description: 'Claude AI 세션을 팀 단위로 자동 수집·검색. 대화 히스토리를 지식 자산으로 보존',
    icon: (
      <svg
        className="h-6 w-6"
        viewBox="0 0 24 24"
  ...
  },
  {
    ...
    description: '같은 파일/모듈을 동시에 작업 중인 팀원을 감지하고, 머지 전 충돌 사전 경고',
    icon: (
      <svg
        className="h-6 w-6"
        viewBox="0 0 24 24"
  ...
  },
  {
    ...
    description: '로컬 Claude Code (CLI) 세션을 원클릭으로 팀에 공유. 프로젝트별 자동 그룹핑',
    icon: (
      <svg
        className="h-6 w-6"
        viewBox="0 0 24 24"
...
function FeatureCard({ title, description, icon }: FeatureCardProps) {
  return (
    <div className="rounded-lg border border-border-default bg-surface p-6">
      ...
      <h3 className="mb-2 font-mono text-sm font-medium text-text-primary">{title}</h3>
      <p className="font-mono text-xs leading-relaxed text-text-tertiary">{description}</p>
    </div>
  );
}
