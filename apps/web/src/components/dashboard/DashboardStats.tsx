...
const statItems: readonly {
  ...
  { key: 'conflicts', label: 'Active Conflicts', field: 'activeConflicts', color: 'text-red-400', bgTint: 'bg-red-500/5', icon: WarningIcon, tooltip: "Conflicts with 'detected' status that haven't been resolved yet." },
  { key: 'members', label: 'Active Members', field: 'activeMembers', color: 'text-purple-400', bgTint: 'bg-purple-500/5', icon: UsersIcon, tooltip: 'Team members who created or updated sessions this week.' },
];

export function DashboardStatsView({ stats }: DashboardStatsProps) {
  return (
    <div className="grid grid-cols-4 gap-4">
      {statItems.map(({ key, label, field, color, bgTint, icon: IconComponent, tooltip }) => (
